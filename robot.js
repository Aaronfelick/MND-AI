...
// mockup if WebGL is unavailable.
// ============================================================================
...
try {
  if (!stage || !canvas) throw new Error('robot stage missing');
  init();
} catch (err) {
  console.warn('BizBot 3D robot disabled:', err);
...
function init() {
  ...
  // ── Lights ──
  const hemi = new THREE.HemisphereLight(0xffffff, 0x778899, 1.1);
  scene.add(hemi);
  const key = new THREE.DirectionalLight(0xffffff, 1.6);
  key.position.set(3, 5, 4);
  scene.add(key);
  const warm = new THREE.PointLight(ACCENT, 12, 12);
  ...
  // Torso
  const torso = new THREE.Mesh(new RoundedBoxGeometry(1.55, 1.5, 1.05, 5, 0.3), bodyMat);
  robot.add(torso);
  ...
  // Waist accent stripe
  const waist = new THREE.Mesh(new RoundedBoxGeometry(1.1, 0.18, 0.85, 3, 0.08), accentMat);
  ...
  const head = new THREE.Mesh(new RoundedBoxGeometry(1.75, 1.3, 1.2, 5, 0.34), bodyMat);
  headGroup.add(head);

  const face = new THREE.Mesh(new RoundedBoxGeometry(1.3, 0.82, 0.12, 4, 0.06), screenMat);
  ...
  robot.add(thruster);
  const thrusterLight = new THREE.PointLight(ACCENT, 6, 4);
  ...
  function applyTheme() {
    ...
    hemi.intensity = isDarkMode() ? 0.7 : 1.1;
    warm.intensity = isDarkMode() ? 20 : 12;
  }
  applyTheme();
  ...
  function animate() {
    ...
    thrusterLight.intensity = 5 + Math.sin(t * 17) * 1.5;
